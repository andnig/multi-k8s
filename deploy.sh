...
docker build -t andnig/multi-server:latest -t andnig/multi-server:$SHA -f ./server/Dockerfile ./server
docker build -t andnig/multi-worker:latest -t andnig/multi-worker:$SHA -f ./worker/Dockerfile ./worker
